#!/usr/bin/env bash
# One-shot cloud dispatch payload for scripts/_diag_ar4_joint_limits_readback.py
# (2026-07-28 follow-up to the ar4-joint-tracking-closed-loop-fix task).
# Uses the container + GCS-cached-asset path, same recipe as
# scripts/_cloud_ar4_tracking_fix_confirm.sh, but only runs the tiny
# joint-limits readback (no cube, no phases, no camera - a few seconds of
# actual sim time) instead of the full sweep/grasp task, since the only
# open question left is a single hard number: what is joint_2's REAL
# baked-in position limit, vs. scripts/ar4_graspable_workspace.py's
# assumed (-42, 90)deg.
#
...
step "[4/4] _diag_ar4_joint_limits_readback.py"
T4_START=$(date +%s)
sudo docker run --rm --gpus all --network host --entrypoint bash \
  ...
  -w /workspace/rl \
  "$IMAGE" \
  -c "/workspace/isaaclab/isaaclab.sh -p scripts/_diag_ar4_joint_limits_readback.py --headless" \
  2>&1 | tee "$HOME/joint_limits_readback.log"
check "${PIPESTATUS[0]}" "_diag_ar4_joint_limits_readback.py (containerized)"
T4_END=$(date +%s)
echo "TIMING run_sec=$((T4_END - T4_START))"

sudo chown -R "$(id -u):$(id -g)" "$HOME/rl/logs" 2>/dev/null || true

GCS_DEST="gs://rl-manipulation-hks-runs/ar4-joint-limits-readback/$(date -u +%Y%m%d-%H%M%S)/"
echo "GCS_DEST=${GCS_DEST}"
gsutil -q cp "$HOME/joint_limits_readback.log" "${GCS_DEST}" 2>&1 || echo "WARNING: log GCS sync failed (non-fatal)"
