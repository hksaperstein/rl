"""UPDATE 2026-07-24 (ar4-vertical-fixed-gripper-recheck task): this is a
DELIBERATE, UNMODIFIED-METHODOLOGY re-run of scripts/grasp_demo_v2.py
exactly as it existed at commit f9bde3e ("AR4 classical-IK grasp - 4 real
bugs found") - position-only IK (command_type="position", no orientation
lock), CUBE_POS_W=(0.0, 0.275, 0.009), GRASP_AT_HEIGHT=0.009 - the
configuration that achieved the genuine, reproducible 10.5mm/1.8mm
(grasp/pregrasp) positioning precision documented in that commit and in
kb/wiki/concepts/ar4-vs-franka-root-cause-comparison.md's 2026-07-22
(ar4-grasp-ik-precision) UPDATE. That run predates d59595a (the jaw2
mirroring-sign fix) and the later jaw2 collision-mesh symmetry fix
(2026-07-24, ar4-jaw-contact-sensor-hypothesis task) - at the time, EE
positioning was excellent but the gripper's jaws collapsed onto the same
point instead of spreading into a real pincer. This script is the SAME
positioning methodology run against the CURRENT, fully-fixed asset/gripper,
with jaw1/jaw2-vs-cube contact-force logging added (the only functional
change from the f9bde3e version - that commit's own script had no contact
sensor readout at all) to directly test whether now-correct positioning +
now-working gripper finally produces real two-sided contact and a lift.

Deliberately NOT using the later command_type="pose" (orientation-locked)
mechanism introduced in 6ca9a1d and everything downstream of it (tilt,
reach sweeps, etc.) - that mechanism was confirmed, extensively, in this
project's own kb history to hit a genuine AR4 joint_3 (elbow) hard-limit
wall when holding a canonical vertical wrist at this cube's true ~9mm
grasp height (tested at 3 reach distances, confirmed against the vendor
hardware spec as a real limit, not an import bug - see this kb article's
2026-07-22 "ar4-grasp-orientation-fix" and "ar4-tilt-fix" UPDATEs). This
script's own position-only IK has no such orientation lock, so the arm's
redundant null space is free to pick whatever wrist orientation reaches
the target position - which is exactly why it could reach the true 9mm
height with sub-cm precision when the later orientation-locked approach
could not. See this file's own original (2026-07-22) docstring below for
the full original investigation, kept verbatim for history.
...
VIDEO_PATH = os.path.join(LOG_DIR, "videos", "ar4_grasp_vertical_position_ik_fixed_gripper.mp4")
        ...
        for phase_idx, (duration, target_q, gripper_cmd) in enumerate(PHASES):
            ...
            for i in range(duration):
                ...
                if phase_idx in (3, 4, 5) and i % 20 == 0:
                    ...
                    # 2026-07-24 (ar4-vertical-fixed-gripper-recheck task):
                    # jaw-vs-cube contact force, added here (absent in the
                    # f9bde3e version this script otherwise reproduces
                    # exactly) - both sensors are filtered against the Cube
                    # prim only (tasks/ar4/pickplace_mirror_env_cfg.py), so
                    # a nonzero reading is unambiguously real cube contact,
                    # matching this project's own standing verification
                    # standard (Experiment 16: contact-force ground truth
                    # over eyeballed video/cube-z alone).
