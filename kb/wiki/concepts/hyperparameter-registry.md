...
|---|---|---|
| `episode_length_s` | `20.0` | Was `5.0` (copied from `pickplace_mirror_env_cfg.py`'s single-object lift task). Run 1 (5.0s) showed episodes always hitting the timeout with `goal_reached` peaking then declining; re-derived from Isaac Lab's own reference tasks, which scale episode length with task *structure*: Reach 12.0s, Lift 5.0s, Cabinet 8.0s, Stack (sequential multi-stage, the closest analog) 30.0s. External literature on long-horizon/multi-stage manipulation RL was checked too, not just Isaac Lab's own tasks: Relay Policy Learning's hierarchical fixed-step budgets, SLIM's multi-stage pick-and-place benchmark, and Meta-World's 500-step-per-episode convention all use proportionally longer horizons for multi-stage tasks, and 20.0s sits inside the range Isaac Lab's own tasks establish. Run 2 (20.0s) converged and held `goal_reached` at ~0.60 — see ROADMAP.md's Experiment 25 entry ("Training run 1"/"Training run 2") for the full comparison. |
