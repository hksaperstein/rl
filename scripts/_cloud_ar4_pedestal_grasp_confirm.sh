#!/usr/bin/env bash
# One-shot cloud dispatch payload for the ar4-pedestal-ground-clearance-fix
...
# below copied verbatim from scripts/_cloud_ar4_tracking_fix_confirm.sh
# (itself copied from scripts/_cloud_ar4_container_pipeline.sh's steady-state
# steps). The AR4 USD asset itself is UNCHANGED by this task (the pedestal
# is a plain isaaclab CuboidCfg static prim, not baked into the robot USD),
# so the existing GCS asset cache is still valid as-is - no rebuild/reupload
# needed.
#
# Single task, one instance: scripts/ar4_pedestal_grasp_confirm.py - full
# grasp+lift re-attempt at 3 genuinely distinct validation points
# (Q0_bearing95/Q1_bearing108/Q2_bearing80), now that the cube rests on a
# 40mm static pedestal instead of directly on the ground (the 2026-07-28
# "ar4-joint2-ground-clearance-fix task" finding: a ground-level cube is
# genuinely ungraspable anywhere in AR4's real workspace). Uses the
# already-validated settle_to_joint_pose closed-loop primitive at the
# baseline 4000/200 gain (already proven safe/effective - not re-swept here)
# - no camera (numeric-only, fast, avoids camera-render risk on this
# exploratory confirm run; a separate camera-enabled script produces the
# demo video only if this one confirms a real grasp+lift).
#
...
step "[4/4] ar4_pedestal_grasp_confirm.py (full grasp+lift re-attempt, cube on pedestal, Q0/Q1/Q2)"
T4_START=$(date +%s)
sudo docker run --rm --gpus all --network host --entrypoint bash \
  ...
  -w /workspace/rl \
  "$IMAGE" \
  -c "/workspace/isaaclab/isaaclab.sh -p scripts/ar4_pedestal_grasp_confirm.py --headless" \
  2>&1 | tee "$HOME/pedestal_grasp_confirm.log"
check "${PIPESTATUS[0]}" "ar4_pedestal_grasp_confirm.py (containerized)"
T4_END=$(date +%s)
echo "TIMING run_sec=$((T4_END - T4_START))"

if grep -q "^Traceback (most recent call last):" "$HOME/pedestal_grasp_confirm.log"; then
  echo "WARNING: a Python Traceback was found in pedestal_grasp_confirm.log despite exit 0 above - inspect the log, this may NOT be a real pass." >&2
fi
if ! grep -q "^FINAL MULTI-POINT SUMMARY" "$HOME/pedestal_grasp_confirm.log"; then
...
GCS_DEST="gs://rl-manipulation-hks-runs/ar4-pedestal-grasp-confirm/$(date -u +%Y%m%d-%H%M%S)/"
echo "GCS_DEST=${GCS_DEST}"
echo "=== GCS sync (log) ==="
gsutil -q cp "$HOME/pedestal_grasp_confirm.log" "$HOME/download_asset.log" "${GCS_DEST}" 2>&1 || \
