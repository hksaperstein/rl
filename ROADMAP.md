...
## Task 7 + FINAL VERDICT: AR4 Franka-fixes transfer — H_ar4_relative FALSIFIED, the fix does not transfer to AR4 (2026-07-21)

Closing verdict for `ar4-franka-fixes-transfer` (plan:
...
testing whether Franka's own confirmed `RelativeJointPositionActionCfg`
fix (`kb/wiki/experiments/d8-antipodal-grasp-quality.md`'s H_relative,
CONFIRMED 3/3 seeds) transfers to AR4's historical, never-cleanly-
root-caused Experiment 26 null (`cube_reached_goal` exact `0.0000`), on
the freshly-fixed AR4 gripper asset (jaw2 mimic-limit fix `64ab5cc` +
this workstream's own jaw2 command-sign fix `928af41`). Full technical
write-up: `kb/wiki/concepts/ar4-vs-franka-root-cause-comparison.md`'s new
"H_ar4_relative transfer test (2026-07-21 follow-up)" section.

All 30 checkpoints (5 per run × 3 seeds × 2 conditions — A2 = absolute
joint-space [Experiment 26 unmodified, freshly retrained on the fixed
asset]; B = relative/delta joint-space) trained to completion. Critic
divergence (the pre-authorized `clip_actions=5.0` contingency, matching
Franka's own real precedent) did NOT occur in any of the 6 runs
(`Loss/value_function` max 0.0055-0.1274 across all runs, nowhere near
Franka's own `181→inf` signature).

| Condition | Seed | contact_freq @iter1499 | antipodal_freq @iter1499 | ever_lifted | cube_reached_goal (all 1500 iters) |
|---|---|---|---|---|---|
...
| B (relative)  | 7   | **0.9751** (real bilateral jaw contact, ~0.17-0.27N, confirmed not a sensor artifact) | 0.0000 | 0.0000 | 0.0 |

`cube_reached_goal` is exact `0.0` as the last, max, AND min value across
all 1500 iterations in all 6 runs — a flat zero for the entire behavioral-
bar trajectory, no exceptions. **Honest gap, flagged rather than glossed
over:** unlike Franka's own H_relative write-up, the intermediate-
checkpoint (iter 0/100/300/700) `contact_freq`/`antipodal_freq` values
Task 6 measured are not reproduced here — this closing task's own handoff
carried forward only final-checkpoint (iter 1499) values, and the raw
per-checkpoint diagnostic artifacts exist only on this run's now-torn-down
cloud instance/GCS sync, not locally. This does not change the verdict
(the pre-registered rule only requires the final-checkpoint bar in ≥2/3
seeds), but it does mean this entry cannot assert the exact *shape* of the
missing curve the way Franka's own three-way comparison could.

**Verdict, per the pre-registered rule (falsified if ≥2/3 Condition-B
seeds hit the exact-zero bar): H_ar4_relative is FALSIFIED.** Seeds
42/123 hit the bar cleanly; seed 7 is a genuine, confirmed partial
exception (real contact, zero antipodal fraction, zero goal-reach) that
does not itself prevent falsification (2/3 is sufficient). Condition A2
(fixed asset, absolute joint-space) reproduces the identical all-zero
`cube_reached_goal` pattern as both the original pre-asset-fix Condition A
and the historical Experiment 26 null
([[experiment-26-gripper-reintroduction]]) — fixing BOTH real gripper
asset defects found this session did NOT, by itself, resolve AR4's
grasp-discoverability problem. Unlike Franka's own H_relative
(`kb/wiki/experiments/d8-antipodal-grasp-quality.md`), whose Condition-B
seeds reproduced task-space's own monotonic-rise curve shape almost
exactly, AR4's Condition B shows no such rise in 2/3 seeds, and the one
exception (seed 7) never crosses into real antipodal geometry.

**3-signature jaw-mimic classification** (all 6 runs): Signature 1
(near-zero contact — exploration/action-space-level failure) dominates
5/6 runs — all of A2, plus condition-B seeds 42/123. Signature 2 (nonzero
contact, zero antipodal fraction — consistent with the jaw-mimic-vs-
actuator dynamics conflict this same workstream found: jaw2 pinned near
its hard limits regardless of commanded target, so real contact happens
but never in a graspable configuration) appears cleanly in condition-B
seed 7 only. Signature 3 (contact + antipodal + still no lift) was never
observed — antipodal contact essentially never occurs at all outside
that one case, so "contact+antipodal+no lift" has no opportunity to
apply here.

**Known limitation:** close-up video review was NOT performed for
seed 7's exception — `scripts/graspgoal_closeup_video.py` is hardcoded to
the absolute-action demo env cfg and would need modification to load a
relative-action checkpoint. The seed 7 finding rests on raw contact-
force/antipodal-angle data cross-checked against the literal training-time
`antipodal_grasp_bonus` reward math, not a watched video — a real
verification-depth gap, not equivalent to the video review other results
in this project have had.

**North Star relevance:** Franka's own H_relative result mattered
specifically because a genuinely joint-space fix (no arm-specific IK
layer) resolved an analogous collapse — evidence against needing an
IK/task-space layer as a hidden prerequisite for the "drop in a new arm,
train immediately" bar. Since H_ar4_relative is FALSIFIED here, that
evidence does **not** extend to AR4 — the identical fix, on a second arm,
does not reproduce the win. This narrows what actually explains AR4's own
problem back toward asset-level defects rather than away from them; it
neither supports nor refutes the North Star's cross-arm bar on its own.
AR4's own prior positive result under task-space/IK control
([[experiment-11-taskspace-ik]]) remains the one condition on this
platform where a nonzero antipodal signal was ever observed at all — this
plan deliberately did not retest that condition, since Experiment 26's
absolute-joint null was the more direct analogue of Franka's own
falsified H_joint condition.

**Honest next candidate direction (not started — AR4 investigation is
not the active priority while the Franka pivot is underway, per
CLAUDE.md):** the fix does not transfer; AR4's null is not explained by
the same joint-space-action mechanism that explained Franka's. The
asset-level hypotheses this session already surfaced — the jaw-mimic-vs-
actuator dynamics conflict (jaw2 pinned at hard limits regardless of
commanded target) and the still-unverified jaw collision geometry — and/or
the still-unresolved classical-IK 17-27mm positioning miss remain the more
likely explanations, consistent with the original Franka-pivot rationale.
Concrete next hypothesis, logged to `BACKLOG.md` as flagged-but-deferred,
**not** executed here: test jaw-mimic vs. independent-actuator by
disabling the mimic constraint entirely and re-running Condition B once.

**New infra gap found** (folded into `docs/cloud/dispatch-checklist.md`):
mid-training (condition-B seed 42, ~iteration 1461/1500), Kit's own
hang-watchdog popped a blocking `zenity` "Kit appears to be hanging" GUI
dialog with nobody present to click it, freezing the run indefinitely — a
new, distinct failure mode from the previously-documented "hangs quietly
during teardown" pattern. Recovered by killing the dialog process, then
(when that alone didn't unstick it) `kill -9`-ing the training process and
resuming from `model_1450.pt` via `--checkpoint` to reach `model_1499.pt`
cleanly — no data lost.

**Bugs found/fixed this workstream:** gripper jaw2 mimic-limit mismatch
(`64ab5cc`) and jaw2 command-sign bug (`928af41`) — see
`kb/wiki/concepts/ar4-vs-franka-root-cause-comparison.md` for full detail
on both.

**Cost:** ≈$2.07 cumulative against the plan's $10 cap (Tasks 1-4 ≈$1.96,
Task 5/6 cloud spend ≈$0.11 from two brief aborted cloud instances before
pivoting to desktop). Well under cap. Full teardown verified.
