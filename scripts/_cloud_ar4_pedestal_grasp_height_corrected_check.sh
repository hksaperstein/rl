#!/usr/bin/env bash
# One-shot cloud dispatch payload for the ar4-pedestal-grasp-height-fix task
# (2026-07-28, direct continuation of the ar4-grasp-trivial-friction-check
# task, which confirmed the capstone grasp's real cause is grasp HEIGHT -
# jaws closing on the cube's TOP FACE rather than straddling its vertical
# CENTER). Runs scripts/ar4_pedestal_grasp_height_corrected_check.py, the
# height-corrected re-attempt, at a lowered grasp joint config derived by a
# Pi-local pure-FK search (see that script's own module docstring for the
# full derivation). Same container + GCS-cached-asset path as
# scripts/_cloud_ar4_pedestal_grasp_trivial_check.sh (asset itself is
# unchanged by this task - only grasp joint-angle constants and camera env
# cfg reused as-is), including the same libnvidia-gl driver-library install
# + GPU/Vulkan sanity checks (this script also renders 2 cameras throughout).
#
...
step "[4/4] ar4_pedestal_grasp_height_corrected_check.py (single point, close-up+elbow video, corrected grasp height)"
T4_START=$(date +%s)
sudo docker run --rm --gpus all -e NVIDIA_DRIVER_CAPABILITIES=all --network host --entrypoint bash \
  ...
  -w /workspace/rl \
  "$IMAGE" \
  -c "PYTHONUNBUFFERED=1 /workspace/isaaclab/isaaclab.sh -p scripts/ar4_pedestal_grasp_height_corrected_check.py --headless" \
  2>&1 | tee "$HOME/pedestal_grasp_height_corrected_check.log"
check "${PIPESTATUS[0]}" "ar4_pedestal_grasp_height_corrected_check.py (containerized)"
T4_END=$(date +%s)
echo "TIMING run_sec=$((T4_END - T4_START))"

if grep -q "^Traceback (most recent call last):" "$HOME/pedestal_grasp_height_corrected_check.log"; then
  echo "WARNING: a Python Traceback was found in pedestal_grasp_height_corrected_check.log despite exit 0 above - inspect the log, this may NOT be a real pass." >&2
fi
if ! grep -q "^VERDICT \[" "$HOME/pedestal_grasp_height_corrected_check.log"; then
...
GCS_DEST="gs://rl-manipulation-hks-runs/ar4-pedestal-grasp-height-corrected-check/$(date -u +%Y%m%d-%H%M%S)/"
echo "GCS_DEST=${GCS_DEST}"
echo "=== GCS sync (log + video/frames) ==="
gsutil -q cp "$HOME/pedestal_grasp_height_corrected_check.log" "$HOME/download_asset.log" "${GCS_DEST}" 2>&1 || \
  echo "WARNING: log GCS sync failed (non-fatal)"
gsutil -m cp -r "$HOME/rl/logs/videos/ar4_pedestal_grasp_height_corrected_check" "${GCS_DEST}" 2>&1 || \
