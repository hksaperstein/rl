#!/usr/bin/env bash
# One-shot cloud setup + run for the ar4-grasp-ik-convergence-tightening
# task (2026-07-24). Builds Isaac Sim/Isaac Lab + a fresh AR4 USD asset
# from scratch (desktop unreachable, no persisted asset/GCS mirror
# exists), then runs the instrumented scripts/grasp_demo_v2.py at the
# best-known kinematic configuration this investigation has found (65deg
# tilt, reach=0.36m - the middle of the confirmed-flat 0.30-0.36m plateau,
# picked over 0.30m/0.39m for having the best residual within the
# plateau (9.38mm) while not sitting at either edge - see kb/wiki/concepts/
# ar4-vs-franka-root-cause-comparison.md's 2026-07-23 ar4-capstone-grasp
# UPDATE's own reach-sweep table), WITH the new --grasp-deep-polish-steps/
# --grasp-pos-threshold/--grasp-rot-threshold flags (this task's own
# addition to grasp_demo_v2.py, commit bc466a2) to test whether the
# ~9.5mm/4.2deg residual this configuration converges to shrinks further
# with substantially more solver effort and a tighter convergence bound,
# or is already a genuine local-optimum plateau.
#
...
# check - cloud, not desktop" section) and the ar4-capstone-grasp/
# ar4-jaw-bisector-hypothesis tasks' own "Setup" sections - reused here
# (this is the third task in a row to need this exact recipe) rather than
# re-derived from memory again, per those tasks' own flagged follow-up
# ("Not yet built: a pre-baked VM image ...").
#
...
GCS_DEST="gs://rl-manipulation-hks-runs/ar4-grasp-ik-convergence-tightening/$(date -u +%Y%m%d-%H%M%S)/"
...
step "[6/6] run grasp_demo_v2.py at reach=0.36m/tilt=65deg with the new deep-polish pass"
# reach=0.36m, tilt=65deg: the middle of the confirmed-flat 0.30-0.36m
# plateau this investigation found (2026-07-23 ar4-capstone-grasp task),
# picked over 0.30m/0.39m for the best in-plateau residual (9.38mm) without
# sitting at either tested edge. --grasp-deep-polish-steps/--grasp-pos-
# threshold/--grasp-rot-threshold are this task's own new flags (commit
# bc466a2): one extra polish_from_seed pass at GRASP's full-precision
# target after the normal descent converges, continuing live (no
# teleport) with a much larger iteration budget (6000 steps, vs the
# descent's own 400-step/150-stagnation-step per-sub-step budget) and a
# meaningfully tighter convergence bound (1mm/0.01rad, vs the module
# default 3mm/0.05rad) than any prior pass at this configuration has used
# - testing directly whether the previously-found ~9.5mm/4.2deg residual
# shrinks with more solver effort or is already a genuine plateau.
...
# apply to cloud dispatch). Runs regardless of any check-logged FAIL
# above, since each of those has independently succeeded in prior
# sessions and the real cost of trying anyway is small compared to losing
# the whole ~30-45min setup investment on a false alarm.
PYTHONUNBUFFERED=1 "$HOME/IsaacLab/isaaclab.sh" -p scripts/grasp_demo_v2.py --headless \
    ...
    --grasp-deep-polish-steps 6000 --grasp-deep-polish-stagnation-steps 2000 \
    --grasp-pos-threshold 0.001 --grasp-rot-threshold 0.01 \
    --video-suffix convtighten_r036_t65_deep \
    2>&1 | tee "$HOME/grasp_convtighten_run.log"
check "${PIPESTATUS[0]}" "grasp_demo_v2.py deep-polish convergence-tightening run"
...
gsutil -q cp "$HOME/grasp_convtighten_run.log" "${GCS_DEST}logs/grasp_convtighten_run.log" 2>&1 || echo "WARNING: GCS sync of grasp_convtighten_run.log failed (non-fatal)"
