#!/usr/bin/env bash
# One-shot cloud dispatch payload for the ar4-joint-tracking-diagnostic task
# (2026-07-27). Uses the container + GCS-cached-asset path
# (docs/cloud/dispatch-checklist.md's preferred AR4 cloud recipe) - steps
# below copied verbatim (same reasoning as prior _cloud_ar4_*.sh payloads)
# from scripts/_cloud_ar4_container_pipeline.sh's own steady-state steps.
#
# Single task, one instance: scripts/ar4_joint_tracking_diagnostic.py -
# commands the arm DIRECTLY (no IK) to the FK-computed P0 GRASP_Q with the
# cube parked far away (no possible obstruction), measures commanded-vs-
# achieved per-joint tracking error under BOTH the current shipped arm
# actuator gains (stiffness=40/damping=4) and the test-local boosted
# override (4000/200) already used by every other confirm script in this
# investigation, plus achieved-vs-FK-predicted gripper pinch-point/
# gripper_base_link world position.
#
...
# 2026-07-27 finding (prior dispatches): do NOT install
# libnvidia-gl-<major>-server - it breaks an already-working nvidia-smi/CUDA.
# This script is camera-free (no rendering needed at all), so there is no
# tradeoff here to even weigh.
...
# --- [4/4] THE ACTUAL DIAGNOSTIC -------------------------------------------
step "[4/4] ar4_joint_tracking_diagnostic.py (commanded-vs-achieved joint tracking, no cube obstruction, both gain regimes)"
T4_START=$(date +%s)
sudo docker run --rm --gpus all --network host --entrypoint bash \
  ...
  -w /workspace/rl \
  "$IMAGE" \
  -c "/workspace/isaaclab/isaaclab.sh -p scripts/ar4_joint_tracking_diagnostic.py --headless" \
  2>&1 | tee "$HOME/joint_tracking_diagnostic.log"
check "${PIPESTATUS[0]}" "ar4_joint_tracking_diagnostic.py (containerized)"
T4_END=$(date +%s)
echo "TIMING diagnostic_run_sec=$((T4_END - T4_START))"

if grep -q "^Traceback (most recent call last):" "$HOME/joint_tracking_diagnostic.log"; then
  echo "WARNING: a Python Traceback was found in joint_tracking_diagnostic.log despite exit 0 above - inspect the log, this may NOT be a real pass." >&2
fi
if ! grep -q "^FINAL SUMMARY (both regimes)" "$HOME/joint_tracking_diagnostic.log"; then
  echo "WARNING: no 'FINAL SUMMARY (both regimes)' line found - the script likely did not reach its own final summary." >&2
fi

sudo chown -R "$(id -u):$(id -g)" "$HOME/rl/logs" 2>/dev/null || true

GCS_DEST="gs://rl-manipulation-hks-runs/ar4-joint-tracking-diagnostic/$(date -u +%Y%m%d-%H%M%S)/"
echo "GCS_DEST=${GCS_DEST}"
echo "=== GCS sync (log) ==="
gsutil -q cp "$HOME/joint_tracking_diagnostic.log" "$HOME/download_asset.log" "${GCS_DEST}" 2>&1 || \
...
echo "diagnostic_run_sec=$((T4_END - T4_START))"
echo "GCS_DEST=${GCS_DEST}"
echo "ALL DONE."
