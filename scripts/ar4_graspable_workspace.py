...
def batch_fk_link6(joint_values: dict[str, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for j in _chain:
        ...
        if j.joint_type == "revolute":
            ...
            raise ValueError(f"batch_fk_link6 only handles revolute joints in this chain, got {j.joint_type}")
...
def run_sweep(n_stage_a: int, n_joint1_steps: int, seed: int = 0, chunk_size: int = 200_000):
    ...
    while n_done < n_stage_a:
        ...
        mask_a = (
            (np.abs(height_err_a) <= HEIGHT_TOL_M) & (tilt_deg_a <= TILT_TOL_DEG) & (margin_2to6_a >= MARGIN_MIN_RAD)
        )
...
if __name__ == "__main__":
    ...
    if result is None:
        print("No graspable configs found at all with the current filter thresholds - see script constants.")
        sys.exit(1)
