#!/usr/bin/env bash
# One-shot cloud dispatch payload for the ar4-cartesian-fingertip-correction
# task (2026-07-28). Uses the container + GCS-cached-asset path
# (docs/cloud/dispatch-checklist.md's preferred AR4 cloud recipe) - steps
# below copied verbatim from scripts/_cloud_ar4_pedestal_grasp_confirm.sh.
# The AR4 USD asset itself is UNCHANGED by this task (the Cartesian
# correction primitive is pure Isaac-Lab-API/torch logic, not a USD edit),
# so the existing GCS asset cache is still valid as-is - no rebuild/reupload
# needed.
#
# Single task, one instance: scripts/ar4_cartesian_grasp_confirm.py - full
# grasp+lift re-attempt at the same 3 pedestal-corrected validation points
# (Q0_bearing95/Q1_bearing108/Q2_bearing80) as the pedestal-fix task, now
# with an added Cartesian outer-loop fingertip correction
# (tasks/ar4/joint_tracking.py's new settle_to_cartesian_pose) between the
# joint-space GRASP-OPEN-SETTLE and the CLOSE phase, targeting the ~6-7mm
# residual pinch-point discrepancy the pedestal-fix task's own
# settle_to_joint_pose could not close (joint-space convergence does not
# imply Cartesian convergence over a ~0.5m lever arm). No camera
# (numeric-only, fast, avoids camera-render risk on this exploratory confirm
# run; a separate camera-enabled script produces the demo video only if
# this one confirms a real grasp+lift).
#
...
step "[4/4] ar4_cartesian_grasp_confirm.py (full grasp+lift re-attempt, Cartesian-corrected fingertip, Q0/Q1/Q2)"
T4_START=$(date +%s)
sudo docker run --rm --gpus all --network host --entrypoint bash \
  ...
  -w /workspace/rl \
  "$IMAGE" \
  -c "/workspace/isaaclab/isaaclab.sh -p scripts/ar4_cartesian_grasp_confirm.py --headless" \
  2>&1 | tee "$HOME/cartesian_grasp_confirm.log"
check "${PIPESTATUS[0]}" "ar4_cartesian_grasp_confirm.py (containerized)"
T4_END=$(date +%s)
echo "TIMING run_sec=$((T4_END - T4_START))"

if grep -q "^Traceback (most recent call last):" "$HOME/cartesian_grasp_confirm.log"; then
  echo "WARNING: a Python Traceback was found in cartesian_grasp_confirm.log despite exit 0 above - inspect the log, this may NOT be a real pass." >&2
fi
if ! grep -q "^FINAL MULTI-POINT SUMMARY" "$HOME/cartesian_grasp_confirm.log"; then
...
GCS_DEST="gs://rl-manipulation-hks-runs/ar4-cartesian-grasp-confirm/$(date -u +%Y%m%d-%H%M%S)/"
echo "GCS_DEST=${GCS_DEST}"
echo "=== GCS sync (log) ==="
gsutil -q cp "$HOME/cartesian_grasp_confirm.log" "$HOME/download_asset.log" "${GCS_DEST}" 2>&1 || \
